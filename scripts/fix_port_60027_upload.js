const fs = require('fs');
const path = require('path');
const { Client } = require('ssh2');

const password = process.env.DEPLOY_SSH_PASSWORD;
if (!password) {
  console.error('DEPLOY_SSH_PASSWORD required');
  process.exit(1);
}

const remotePath = '/tmp/portfwd_60027.py';
const localPath = path.join(__dirname, 'portfwd_60027.py');
const content = fs.readFileSync(localPath, 'utf8');

const conn = new Client();
conn.on('ready', async () => {
  const sftp = await new Promise((resolve, reject) => {
    conn.sftp((err, s) => (err ? reject(err) : resolve(s)));
  });
  await new Promise((resolve, reject) => {
    const stream = sftp.createWriteStream(remotePath);
    stream.on('error', reject);
    stream.on('close', resolve);
    stream.end(content);
  });

  conn.exec(`
pkill -f '${remotePath}' 2>/dev/null || true
nohup python3 ${remotePath} >/tmp/portfwd_60027.log 2>&1 &
sleep 1
ss -lntp | grep -E '60027|24009' || true
cat /tmp/portfwd_60027.log 2>/dev/null || true
curl -s -m 5 -o /dev/null -w "host60027:%{http_code}\\n" http://127.0.0.1:60027/ || true
curl -s -m 10 "http://127.0.0.1:60027/v1/jangsu/getBoundsList?minx=127.45&miny=35.60&maxx=127.58&maxy=35.70" | head -c 160; echo
`, (err, stream) => {
    stream.on('data', (d) => process.stdout.write(d));
    stream.stderr.on('data', (d) => process.stderr.write(d));
    stream.on('close', () => conn.end());
  });
}).connect({ host: '61.35.161.124', port: 60014, username: 'root', password });
conn.on('error', (e) => {
  console.error(e);
  process.exit(1);
});
